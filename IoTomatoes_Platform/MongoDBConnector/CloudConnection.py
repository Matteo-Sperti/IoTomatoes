...
class MongoConnection():
	...
	def insertTruckData(self, CompanyName, TruckID, data):
		...
		dict_=self.client[CompanyName][CollectionName].find_one({"_id":TruckID})
		if dict_ == None:
			...
			if (data["e"][0]["v"]["timestamp"] - dict_["e"][0]["v"]["timestamp"][-1]) > 3600 :
				...
				data["e"][0]["v"]["longitude"]=[data["e"][0]["v"]["longitude"]]
				data["e"][0]["v"]["timestamp"]=[data["e"][0]["v"]["timestamp"]]
				self.client[CompanyName][CollectionName].update_one({"_id":TruckID}, {"$set": data})
			else:
			
				dict_["e"][0]["v"]["latitude"].append(data["e"][0]["v"]["latitude"])
				dict_["e"][0]["v"]["longitude"].append(data["e"][0]["v"]["longitude"])
				dict_["e"][0]["v"]["timestamp"].append(data["e"][0]["v"]["timestamp"])

				self.client[CompanyName][CollectionName].update_one({"_id":TruckID}, {"$set": dict_})
	...
	def notify(self,topic, payload):
			...
			try:
				if listTopic[2] == "consumption":
					...
					self.insertDeviceData(listTopic[1],listTopic[2],listTopic[3],listTopic[4],payload)
	...
	def checkNewCompany(self):
			...
			while True:
				try:
					response = requests.get(self.ResourceCatalog_url + "/companies/names")
				...
				for i in range(len(dict)):
					...
					for j in range(len(dict[i]["e"])):
						if dict[i]["e"][j]["name"] == measure:
							...
							avg=sum(dict[i]["e"][j]["value"][indexes[0]:indexes[1]])/len(dict[i]["e"][j]["value"][indexes[0]:indexes[1]])
							lst.append(avg)
				...
				for i in range(len(dict_)):
					...
					for j in range(len(dict_[i]["e"])):
						if dict_[i]["e"][j]["name"] == measure:
							indexes=self.time_period(dict_[i]["e"][j]["timestamp"],start,end)
							lst.extend(dict_[i]["e"][j]["value"][indexes[0]:indexes[1]])
							timestamps.extend(dict_[i]["e"][j]["timestamp"][indexes[0]:indexes[1]])
							unit=dict_[i]["e"][j]["unit"]
					resultDict[dict_[i]["_id"]]={"values":lst,"timestamps":timestamps,"unit":unit}
				return json.dumps(resultDict)
			else:
				raise web_exception(404, "Field not found")
		else:
					...
					try:
						for i in range(len(dict_)):
							...
							if "consumption" in dict_[i].keys():
								indexes=self.time_period(dict_[i]["consumption"]["timestamp"],start,end)
								lst.extend(dict_[i]["consumption"]["consumption_value"][indexes[0]:indexes[1]])
								timestamps.extend(dict_[i]["consumption"]["timestamp"][indexes[0]:indexes[1]])
								unit=dict_[i]["consumption"]["unit"]
							resultDict[CollectionName]={"values":sum(lst),"timestamps":timestamps,"unit":unit}
					except KeyError:
						pass
					if resultDict=={}:
	...
	def getTruckTrace(self,CompanyName,TruckID):
		...
		dict_=self.client[CompanyName]["TruckData"].find_one({"TruckID":TruckID})
		if dict_ != None:
	...
	def getTruckPosition(self,CompanyName):
		...
		returnDict={}
		for i in dict_:
			returnDict[dict_[i]["_id"]]={"latitude":dict_[i]["e"][0]["v"]["latitude"],"longitude":dict_[i]["e"][0]["v"]["longitude"]}
...
class RESTConnector(BaseService):
	...
	def __init__(self,settings:dict):
		...
		self.mongo = MongoConnection(self.ResourceCatalog_url, settings["MongoDB_Url"], settings["PlantDatabaseName"])
